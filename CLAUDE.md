...
1. **Private key never crosses the bridge.** The `SecureKeyring` TurboModule contract has no private-key parameters or return values. Outputs are only `address` / `signedRawTxHex` / `signatureHex` / `mnemonic` (mnemonic only on create/import/backup). Signing happens inside TWCore in native memory; the handle is released immediately after.
